...
public class GlobalExceptionHandler {
    ...
    /**
     * Google OAuth / Gmail API problems. The message is composed by the Google layer specifically
     * to be shown to an administrator and never contains a token, a code or the client secret, so
     * it is passed through as-is rather than replaced with something vaguer.
     */
    @ExceptionHandler(GoogleAuthException.class)
    public ResponseEntity<ApiResponse<Void>> handleGoogleAuth(GoogleAuthException ex) {
        // A revoked grant is the admin's to fix by reconnecting, not a server fault: 400, not 502.
