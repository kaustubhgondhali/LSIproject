...
public interface AcadSequenceRepository extends JpaRepository<AcadSequence, Long> {
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select s from AcadSequence s where s.kind = :kind and s.year = :year")
    Optional<AcadSequence> findForUpdate(@Param("kind") String kind, @Param("year") int year);

    Optional<AcadSequence> findByKindAndYear(String kind, int year);
}
