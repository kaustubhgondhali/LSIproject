...
import com.lordsai.lsi.dto.user.UserDtos.StudentCreated;
...
public class AdminService {
    ...
    private final EbookEntitlementRepository ebookEntitlementRepository;
    ...
    public AdminService(UserRepository userRepository,
                        ...
                        EbookEntitlementRepository ebookEntitlementRepository,
        ...
        this.ebookEntitlementRepository = ebookEntitlementRepository;
    ...
    public DashboardStats dashboard() {
        ...
        return new DashboardStats(
                ...
                ebookRepository.countByStatus(EbookStatus.ACTIVE),
                ebookEntitlementRepository.countByStatus(EntitlementStatus.ACTIVE),
                invoiceRepository.count(),
                communicationLogRepository.countByStatus(CommunicationStatus.FAILED));
    }
    ...
    @Transactional
    public StudentCreated createStudent(CreateStudentRequest req, User actor, String ip) {
        ...
        if (req.wantsCredentials()) {
            EmailDelivery delivery = issueCredentialsSetup(c.user(), c.profile().getStudentId(), actor, ip);
            return new StudentCreated(toStudent(c.user()), delivery.delivered()
                    ? "Student created. Login credentials have been emailed to " + c.user().getEmail() + "."
                    : "Student created, but the credentials email could not be sent (" + delivery.reason()
                            + "). Use \"Send Login Credentials\" on the student page to generate and send a new setup link.",
                    delivery.delivered());
        }

        String link = accountTokenService.issueLink(c.user(), TokenPurpose.ACCOUNT_SETUP);
        EmailDelivery delivery = emailService.sendWelcomeEmail(c.user(), link);
        return new StudentCreated(toStudent(c.user()), delivery.delivered()
                ? "Student created. A password setup email has been sent."
                : "Student created, but the setup email could not be sent (" + delivery.reason()
                        + "). Use \"Resend Setup Email\" on the student page once email is configured.",
                delivery.delivered());
    }

    /**
     * Generates a fresh secure one-time password setup link for an existing student and emails it.
     * Any previous password setup tokens become invalid, all active sessions end.
     * Passwords are never sent by email or logged in plaintext.
     */
    @Transactional
    public String sendLoginCredentials(Long id, User actor, String ip) {
        User student = userService.requireUser(id, Role.STUDENT);
        if (student.getAccountStatus() == AccountStatus.DISABLED) {
            throw new ApiException(HttpStatus.CONFLICT, "Activate the account before sending credentials.");
        }
        String studentId = userService.requireStudentProfile(id).getStudentId();
        EmailDelivery delivery = issueCredentialsSetup(student, studentId, actor, ip);
        return delivery.delivered()
                ? "A secure password setup link has been emailed to " + student.getEmail() + "."
                : "A password setup link was generated, but the email could not be sent (" + delivery.reason()
                        + "). Check Email Settings and try again — each attempt generates a new secure setup link.";
    }

    private EmailDelivery issueCredentialsSetup(User user, String userId, User actor, String ip) {
        ...
        String setupLink = accountTokenService.issueLink(user, purpose);
        user.setAccountStatus(AccountStatus.ACTIVE);
        user.setTokenVersion(user.getTokenVersion() + 1);
        userRepository.save(user);
        sessionService.revokeAll(user.getId(), SessionRevokeReason.PASSWORD_CHANGED);

        String loginUrl = properties.publicBaseUrl().replaceAll("/+$", "") + "/student-login.html"
                + (user.getRole() == Role.ADMIN ? "?portal=admin" : "");
        EmailDelivery delivery = emailService.sendLoginCredentials(user, userId, setupLink, loginUrl);
        auditService.record(actor, delivery.delivered() ? "LOGIN_CREDENTIALS_EMAILED" : "LOGIN_CREDENTIALS_EMAIL_FAILED",
                "User", user.getId(), "Password setup link issued for " + user.getEmail()
    ...
    /**
     * Re-sends the enrollment email (Student ID + a fresh setup link) for a student whose email
     * never arrived. Creates no student and no enrollment; the caller is told whether it was
     * actually delivered so the admin is not left assuming it was.
     */
    @Transactional
    public String resendSetupEmail(Long id, User actor, String ip) {
        User student = userService.requireUser(id, Role.STUDENT);
        EmailDelivery delivery = enrollmentService.resendSetupEmail(student);
        auditService.record(actor, "SETUP_EMAIL_RESENT_BY_ADMIN", "User", id,
                student.getEmail() + ": " + delivery.status(), ip);
        return delivery.delivered()
                ? "Setup email re-sent to " + student.getEmail() + "."
                : "Could not send the email (" + delivery.reason() + "). Please check the mail configuration.";
    }
