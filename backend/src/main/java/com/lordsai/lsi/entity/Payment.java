...
@Table(name = "payments", indexes = {
        ...
        @Index(name = "idx_payments_product", columnList = "product_type, course_id, ebook_id"),
        @Index(name = "idx_payments_verified", columnList = "verified_at")
})
public class Payment extends BaseEntity {
    ...
    /**
     * What was bought. Exactly one of {@link #course} / {@link #ebook} is set, according to
     * {@link #productType}. Every payment row is therefore also the purchase record: student,
     * product, amount, transaction id, status and date live here, and the invoice links back to it.
     */
    ...
    /** Amount in INR, copied from the product price at order time by the backend. */
    ...
    public String productName() {
        return productType == ProductType.EBOOK
                ? (ebook == null ? "Ebook" : ebook.getTitle())
                : (course == null ? "Course" : course.getCourseName());
    }

    public String productCode() {
        return productType == ProductType.EBOOK
                ? (ebook == null ? null : ebook.getEbookCode())
