...
class SuccessStoryTest {
    ...
    void storyAppearsPubliclyOnlyAfterPublishingAndVideoIsRequiredToPublish() throws Exception {
        ...
        @Test
        void multipartCreateRequiresBothFilesAndPublishesUploadedStory() throws Exception {
        mockMvc.perform(multipart("/api/admin/stories")
                        .file(new MockMultipartFile("video", "story.mp4", "video/mp4", new byte[128]))
                .param("category", "STUDENTS")
                .param("title", "Missing cover")
                .header("Authorization", "Bearer " + admin))
            .andExpect(status().isBadRequest())
            .andExpect(jsonPath("$.message").value("Cover image is required."));

        MockMultipartFile cover = new MockMultipartFile("coverImage", "cover.jpg", "image/jpeg",
            new byte[]{(byte) 0xFF, (byte) 0xD8, (byte) 0xFF, 1});
        MockMultipartFile video = new MockMultipartFile("video", "story.mp4", "video/mp4", new byte[128]);
        long id = objectMapper.readTree(mockMvc.perform(multipart("/api/admin/stories")
                                .file(cover).file(video)
                    .param("category", "STUDENTS")
                    .param("title", "  Uploaded student story  ")
                    .param("description", "Optional information")
                    .header("Authorization", "Bearer " + admin))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Story video uploaded successfully."))
                .andExpect(jsonPath("$.data.title").value("Uploaded student story"))
                .andExpect(jsonPath("$.data.thumbnailPath").value(org.hamcrest.Matchers.startsWith("images/")))
                .andExpect(jsonPath("$.data.hasUploadedVideo").value(true))
                .andReturn().getResponse().getContentAsString()).path("data").path("id").asLong();

        api.patch(admin, "/api/admin/stories/" + id + "/publish", Map.of("published", true)).andExpect(status().isOk());
        JsonNode publicStories = api.data(api.get(null, "/api/public/stories?category=STUDENTS"));
        assertThat(publicStories.findValue("videoType").asText()).isEqualTo("UPLOAD");
        mockMvc.perform(get("/api/public/stories/" + id + "/video")).andExpect(status().isPartialContent());
        }
