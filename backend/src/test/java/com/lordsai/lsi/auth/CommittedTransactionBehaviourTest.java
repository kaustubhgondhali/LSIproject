...
class CommittedTransactionBehaviourTest {
    ...
    @Test
    void secondDeviceLoginReturnsFriendly409NotA500() throws Exception {
        ...
        // A student on a second device gets the 409 message, not "Something went wrong".
        api.post(null, "/api/auth/login", Map.of("identifier", "committed@test.local", "password", TestUsers.PASSWORD))
                .andExpect(status().isOk());
        api.post(null, "/api/auth/login", Map.of("identifier", "committed@test.local", "password", TestUsers.PASSWORD))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.message").value("This account is already logged in on another device. Please logout from the other device first."));

        // An admin may re-login (previous session revoked) — must commit cleanly too.
