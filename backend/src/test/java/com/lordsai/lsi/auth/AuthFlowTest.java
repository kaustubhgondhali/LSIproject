...
class AuthFlowTest {
    ...
    @Test
    void studentCanLoginNormallyOnMultipleDevicesWithoutADeviceLimit() throws Exception {
    ...
    @Test
    void administratorUserIdsAreChangedInPlaceAndRemainRoleSeparated() throws Exception {
        testUsers.admin("main-admin@test.local");
        String main = tokenFrom(mockMvc.perform(loginVia("main-admin@test.local", TestUsers.PASSWORD, "ADMIN")).andReturn());
        mockMvc.perform(post("/api/auth/change-user-id").header("Authorization", "Bearer " + main)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"currentUserId\":\"main-admin\",\"newUserId\":\"main.office\",\"currentPassword\":\"" + TestUsers.PASSWORD + "\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("User ID changed successfully."));
        String renamedMain = tokenFrom(mockMvc.perform(loginVia("main.office", TestUsers.PASSWORD, "ADMIN")).andExpect(status().isOk()).andReturn());
        mockMvc.perform(loginVia("main-admin", TestUsers.PASSWORD, "ADMIN")).andExpect(status().isUnauthorized());
        mockMvc.perform(get("/api/admin/dashboard").header("Authorization", "Bearer " + renamedMain)).andExpect(status().isOk());

        String automation = tokenFrom(mockMvc.perform(loginVia("automation", "Automation@123", "AUTOMATION")).andReturn());
        mockMvc.perform(post("/api/auth/change-user-id").header("Authorization", "Bearer " + automation)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"currentUserId\":\"automation\",\"newUserId\":\"office.settings\",\"currentPassword\":\"Automation@123\"}"))
                .andExpect(status().isOk());
        mockMvc.perform(loginVia("office.settings", "Automation@123", "AUTOMATION")).andExpect(status().isOk());
        mockMvc.perform(loginVia("main.office", TestUsers.PASSWORD, "ADMIN")).andExpect(status().isOk());
    }

    @Test
    void administratorUserIdChangeRequiresTheCurrentPasswordAndUniqueId() throws Exception {
        testUsers.admin("first-admin@test.local");
        testUsers.admin("taken-id@test.local");
        String token = tokenFrom(mockMvc.perform(loginVia("first-admin", TestUsers.PASSWORD, "ADMIN")).andReturn());
        mockMvc.perform(post("/api/auth/change-user-id").header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"currentUserId\":\"first-admin\",\"newUserId\":\"taken-id\",\"currentPassword\":\"wrongpass1\"}"))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value("Current password is incorrect."));
        mockMvc.perform(post("/api/auth/change-user-id").header("Authorization", "Bearer " + token)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"currentUserId\":\"first-admin\",\"newUserId\":\"taken-id\",\"currentPassword\":\"" + TestUsers.PASSWORD + "\"}"))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.message").value("User ID is already in use."));
    }
